...
public function doPageHtLogin(){
	...
	if(!empty($record)) {
		...
		if (!empty($account)) {
			...
			//订单统计
			$sql2="select count( case when status=1 then 1 end) as dzf, count( case when status=4 then 1 end) as ywc, count( case when status=7 then 1 end) as ytk from  ".tablename('zh_jdgjb_order')." where uniacid=:uniacid  and seller_id=:seller_id ";
