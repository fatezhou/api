<?php
/**
 * [北通授权系统 System] Copyright (c) 2018 WEBY.CC
 * 北通授权系统 is NOT a free software, it under the license terms, visited http://www.beitog.cn/ for more details.
 */
defined('IN_IA') or exit('Access Denied');
define('HTTP_X_FOR', (isset($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] == '443') ? 'https://' : 'http://');
define('CLOUD_GATEWAY_URL', HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBwL2FwaS5waHA='));
define('CLOUD_GATEWAY_URL_NORMAL', HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBwL2FwaS5waHA='));
...
function cloud_request($url, $post = '', $extra = array() , $timeout = 60) {
    ...
    if (!empty($_W['setting']['cloudip']['ip']) && empty($extra['ip'])) {
        ...
        //$extra['ip'] = base64_decode("YXV0aC5iZWl0b2cuY24=");
...
function cloud_build() {
    ...
    $dat = cloud_request(HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBwL2FwaS5waHA='), $pars);
...
function cloud_download($path, $type = '') {
    ...
    $dat = cloud_request(HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBwL2FwaS5waHA='), $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_cron_create($cron) {
    ...
    $result = cloud_request(HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBwL2FwaS5waHA='), $pars);
...
function cloud_auth_url($forward, $data = array()) {
    ...
    $auth_url = HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBpL2F1dGgucGhwP19fYXV0aD0=') . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master) {
    ...
    $dat = cloud_request(HTTP_X_FOR . base64_decode('YXV0aC5iZWl0b2cuY24vYXBwL2FwaS5waHA='), $pars, array() , 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
