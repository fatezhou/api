...
<ul class="nav nav-tabs">    
    <li class="active"><a href="javascript:void(0);">房价维护</a></li>
</ul>
...
<div class="main" style="margin-top: 0px;">
    ...
    <div class="main" style="margin-top: 0px;">
        <div class="panel panel-default">
           <div class="panel-heading">
                维护房价 (单击修改价格，按回车键确认,只能维护28天内的信息)
            </div>
    ...
    $("#yframe-17").addClass("wyactive");
            ...
            window.get_list  = function(start, end, page) {
                $("#d_list").html("正在加载...");
                $.post("<?php  echo url('site/entry/price',array('m'=>'zh_jdgjb','version_id'=>$version));?>", {ac: 'getDate',  page: page, start: start, end: end}, function (data) {
                    data = eval("(" + data + ")");
                    if (data.result == 1) {
            ...
            window.submitPrice = function(input){
               ...
               if(isNaN(price)){
                ...
                $.post("<?php  echo url('site/entry/editprice',array('m'=>'zh_jdgjb','version_id'=>$version));?>",
                        {ac: 'submitPrice',pid:pid,price:price,dateday:dateday,rid:rid}, function (data) {
                            data = eval("(" + data + ")");
                            if (data.result == 1) {
                            }
                        });
