...
<style>
	...
	.big-menu .panel .panel-heading .wytitle{height: 50px;padding: 15px 15px;width: 140px;}
	...
	.fa-database,.fa-child,.fa-server,.fa-users,.fa-bars,.fa-map-marker,.fa-newspaper-o,.fa-book,.fa-info-circle,.fa-cart-plus,.fa-desktop,.fa-product-hunt,.fa-life-ring,.fa-recycle,.fa-cog,.fa-gift,.fa-binoculars,.fa-star-half-o,.fa-trophy,.fa-key{margin-right: 14px;}
	.fa-user{margin-right: 15px;}
	.fa-compass{margin-right: 17px;}
	.fa-money,.fa-car,.fa-cubes{margin-right: 10px;}
	.fa-book{margin-right: 12px;}
	.fa-comment-o{margin-right: 12px;}
	.fa-university{margin-right: 12px;}
	.fa-graduation-cap{margin-right: 5px;}
</style>

<?php  $res=pdo_get('zh_jdgjb_system',array('uniacid'=>$_W['uniacid']))?>
<div class="header">
    <div class="wheaderbox">
        <img class="header_logo" src="<?php  if($res['link_logo']) { ?><?php  echo tomedia($res['link_logo'])?><?php  } else { ?>../addons/zh_jr/template/images/htlogo.png<?php  } ?>" alt=""/>
    	<span class="header_title">
    		<?php  if($res['link_name']) { ?><?php  echo $res['link_name'];?><?php  } else { ?>后台管理系统<?php  } ?>
		</span>    	
    </div>
       <div class="header_left">        
        <?php  if($_W['role'] == 'founder') { ?>
        <a href="<?php  echo $this->createWebUrl('ptdata')?>"><div class="header_btn">返回首页</div></a>
         <?php  } ?>  
    </div>
...
$(function(){
    setInterval(function(){
               $.ajax({	
                type: "post",
                dataType: "json",
                url: "<?php  echo $_W['siteroot'];?>/app/index.php?i=<?php  echo $_W['uniacid'];?>&c=entry&do=neworder&m=zh_jdgjb&role=<?php  echo $_W['role'];?>&uid=<?php  echo $_W['user']['uid'];?>&uniacid=<?php  echo $_W['uniacid'];?>",
...
<audio id="myaudio" src="../addons/zh_jdgjb/template/images/text2audio.mp3" controls="controls"  hidden="true" ></audio>