			...
			<tr ng-repeat="module in moduleList">
				...
				<td class="vertical-middle <?php  if($_GPC['support'] != 'welcome_support') { ?> table-manage-td <?php  } ?>">
					<div class="link-group">
						...
						<a href="<?php  echo url('module/manage-system/upgrade')?>module_name={{ module.name }}" class="color-red del" ng-if="module.new_version">升级</a>
						<?php  } ?>
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="user-head-info we7-padding-bottom">
		...
		<a ng-if="isFounder == 1 && moduleinfo.cloud_mid != ''" ng-href="{{ 'http://www.maomikj.cn/module-' + moduleinfo.cloud_mid + '.html' }}" target="_blank" class="btn btn-primary pull-right">查看商城详情</a>
		<?php  } ?>
	</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && branch.displayorder >= upgradeInfo.site_branch.displayorder" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr>
			...
			<td colspan="4" class="text-right">
				...
				<a href="http://www.maomikj.cn/module-{{upgradeInfo.id}}.html" ng-if="branch.displayorder > upgradeInfo.site_branch.displayorder || (branch.displayorder == upgradeInfo.site_branch.displayorder && branch.id > upgradeInfo.site_branch.id)" class="color-default view-detail" target="_blank">查看分支详情</a>
			</td>
		</tr>
...
<script>
	require(['fileUploader'], function() {
		angular.module('moduleApp').value('config', {
			...
			'getUpgradeInfoUrl' : "<?php  echo url('module/manage-system/check_upgrade')?>"
	});
